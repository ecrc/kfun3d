...
{
#ifdef __USE_HW_COUNTER
  const struct fd fd = flux->perf_counters->fd;
...
#pragma omp parallel for
  ...
  flux->perf_counters->ctrs->flux.cycles += cycle;
  flux->perf_counters->ctrs->flux.tot.imcR += tot.imcR;
  flux->perf_counters->ctrs->flux.tot.imcW += tot.imcW;
  flux->perf_counters->ctrs->flux.tot.edcR += tot.edcR;
  flux->perf_counters->ctrs->flux.tot.edcW += tot.edcW;
#endif
}
