...
extern void ComputeNewton(GMRES *, void *, /*Vec,*/ double *, double *, Jacobian *);//, Mat);
...
extern GMRES *CreateGMRESTable(const size_t);

//KSPSetUp_GMRES1(GMRES *, size_t);
...
//  CHKERRQ(ierr);
  ...
  GMRES *gmres = CreateGMRESTable(g->c->sz);

//GMRES *) malloc(sizeof(GMRES));
...
//                       }
  ...
  for(i = 0; (i < MAX_TS && fnorm_ratio <= FNORM_RATIO); i++)
  {
    ...
    */
    ComputeNewton(gmres, &ctx/*, q*/, r, x, amat);//, A);
    //CHKERRQ(ierr);
